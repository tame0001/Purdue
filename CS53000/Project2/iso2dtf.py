...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.slider_minclipper = QSlider()
        self.slider_maxclipper = QSlider()
        ...
        self.gridlayout.addWidget(QLabel("Min Gradient"), 5, 2, 1, 1)
        self.gridlayout.addWidget(self.slider_minclipper, 5, 3, 1, 1)
        self.gridlayout.addWidget(QLabel("Max Gradient"), 5, 4, 1, 1)
        self.gridlayout.addWidget(self.slider_maxclipper, 5, 5, 1, 1)
...
class PyQtDemo(QMainWindow):
    ...
    def __init__(self, args, parent=None):
        ...
        self.gradient_range = self.reader_gradient.GetOutput().GetPointData().GetArray(0).GetRange()
        self.gradient_factor = (self.gradient_range[1] - self.gradient_range[0]) / 100

        self.gradient_min = self.gradient_range[0] / self.gradient_factor
        self.gradient_max = self.gradient_range[1] / self.gradient_factor

        self.reader = vtk.vtkXMLImageDataReader()
        self.reader.SetFileName(args.data)
        self.reader.Update()
        self.isoval_range = self.reader.GetOutput().GetPointData().GetArray(0).GetRange()

        self.isovalue_factor = (self.isoval_range[1] - self.isoval_range[0]) / 100
        ...
        self.probe.SetInputConnection(self.iso.GetOutputPort())
        self.probe.SetSourceConnection(self.reader_gradient.GetOutputPort())
        
        self.normal = vtk.vtkPolyDataNormals()
        self.normal.SetInputConnection(self.probe.GetOutputPort())
        ...
        self.minclipper = vtk.vtkClipPolyData()
        self.minclipper.SetInputConnection(self.zclipper.GetOutputPort())
        self.minclipper.SetValue(self.gradient_min * self.gradient_factor)

        self.maxclipper = vtk.vtkClipPolyData()
        self.maxclipper.SetInputConnection(self.minclipper.GetOutputPort())
        self.maxclipper.SetValue(self.gradient_max * self.gradient_factor)
        self.maxclipper.InsideOutOn()
        
        self.iso_mapper = vtk.vtkDataSetMapper()
        self.iso_mapper.SetInputConnection(self.maxclipper.GetOutputPort())
        self.iso_mapper.ScalarVisibilityOn()
        self.iso_mapper.SetScalarRange(self.gradient_range)
        ...
        slider_setup(self.ui.slider_minclipper, self.gradient_min, [0, 100], 10)
        slider_setup(self.ui.slider_maxclipper, self.gradient_max, [0, 100], 10)

    def isovalue_callback(self, val):
        self.isovalue = val
        self.iso.SetValue(0, self.isovalue * self.isovalue_factor)
        # print(self.isovalue * self.isovalue_factor)
    ...
    def minclipper_callback(self, val):
        self.gradient_min = val
        self.minclipper.SetValue(self.gradient_min * self.gradient_factor)
        # print(self.gradient_min * self.gradient_factor)
        self.ui.vtkWidget.GetRenderWindow().Render()

    def maxclipper_callback(self, val):
        self.gradient_max = val
        self.maxclipper.SetValue(self.gradient_max * self.gradient_factor)
        # print(self.gradient_max * self.gradient_factor)
        self.ui.vtkWidget.GetRenderWindow().Render()
...
if __name__ == "__main__":
    ...
    window.ui.slider_minclipper.valueChanged.connect(window.minclipper_callback)
    window.ui.slider_maxclipper.valueChanged.connect(window.maxclipper_callback)
    sys.exit(app.exec_())
