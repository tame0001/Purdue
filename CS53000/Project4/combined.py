...
parser.add_argument('scalar', metavar='scalar', type=str, help='Scalar data file')
...
reader_scalar = vtk.vtkStructuredPointsReader()
reader_scalar.SetFileName(args.scalar)
reader_scalar.Update()
scalar_range = reader_scalar.GetOutput().GetPointData().GetArray(0).GetRange()

iso = vtk.vtkContourFilter()
iso.SetInputConnection(reader_scalar.GetOutputPort())
iso.SetValue(0, 300)
...
iso_mapper.SetInputConnection(normal.GetOutputPort())
iso_mapper.ScalarVisibilityOff()

iso_actor = vtk.vtkActor()
iso_actor.SetMapper(iso_mapper)
iso_actor.GetProperty().SetDiffuseColor(0, 1, 0)
iso_actor.GetProperty().SetOpacity(0.3)
